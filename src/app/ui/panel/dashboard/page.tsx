...
export default function AdminDashboard(): React.JSX.Element {
  ...
  // Helper function to safely get count with error handling and timeout
  const getCountSafe = useCallback(async (
    ...
    try {
      ...
      console.warn(`Error querying ${tableName}:`, error?.message || err);
      return 0;
    }
  }, []);
  ...
  const loadDashboardData = useCallback(async () => {
    try {
      ...
      // Run queries sequentially with proper error handling
  ...
  // GSAP animations - optimized to prevent flickering
  useEffect(() => {
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              <div className="h-8 bg-gray-300 dark:bg-gray-600 rounded-lg w-64 animate-pulse"></div>
              <div className="h-4 bg-gray-200 dark:bg-gray-700 rounded w-96 animate-pulse"></div>
            </div>
            <div className="h-10 bg-blue-300 dark:bg-blue-600 rounded-lg w-24 animate-pulse"></div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            {[...Array(6)].map((_, i) => (
              ...
              >
                ...
                <div className="h-8 bg-gray-400 dark:bg-gray-500 rounded animate-pulse w-3/4"></div>
              </motion.div>
            ))}
          </div>
        </motion.div>
  ...
  const Stat = ({ 
    ...
    useEffect(() => {
      if (!loading && !hasAnimated) {
        ...
        }, 400); // Delay to prevent flickering
    ...
    return (
      ...
      >
        ...
        <div className={`absolute inset-0 bg-gradient-to-r ${scheme.accent} opacity-0 group-hover:opacity-5 transition-opacity duration-500`}></div>
        ...
        <div 
          ...
          style={{ color: scheme.icon.replace('text-', '').replace('-600', '') }}
        />
      </motion.div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-blue-25 to-sky-100 dark:from-neutral-950 dark:via-blue-950 dark:to-sky-900" ref={containerRef}>
