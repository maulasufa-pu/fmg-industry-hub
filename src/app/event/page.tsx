...
/*************************************************
 * FMG Universe — /events (Light + Dark friendly)
 ...
 * - Palette GradientArt BERBEDA dari /academy
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 top-0 z-[60] flex h-14 items-center justify-between px-4 sm:px-8">
      <div className="pointer-events-auto inline-flex items-center gap-2 rounded-full bg-neutral-900/60 px-3 py-1.5 text-white backdrop-blur-md dark:bg-white/10">
        ...
        <span className="text-xs tracking-wide opacity-90">Universe • Events</span>
      </div>
...
function ParallaxField({ container }: { container: React.RefObject<HTMLDivElement | null> }): React.JSX.Element {
  ...
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      {/* Soft radial washes — beda tone dari /academy */}
      <motion.div
        style={reduce ? undefined : { y: ySlow }}
        className="absolute -top-32 -left-24 h-[34rem] w-[34rem] rounded-full bg-gradient-to-br from-rose-400/18 via-orange-300/14 to-lime-300/12 sm:h-[40rem] sm:w-[40rem] sm:-top-40 sm:-left-32 blur-2xl"
      />
      <motion.div
        style={reduce ? undefined : { y: yMed }}
        className="absolute -bottom-36 -right-24 h-[32rem] w-[32rem] rounded-full bg-gradient-to-tr from-cyan-400/18 via-sky-300/14 to-indigo-300/12 sm:h-[38rem] sm:w-[38rem] sm:-bottom-44 sm:-right-28 blur-2xl"
      />
      ...
      {!reduce && !isMobile && (
        <>
          ...
          <motion.div
            ...
            className="absolute right-[12%] top-[60%] h-52 w-52 rounded-full bg-gradient-to-br from-amber-300/45 via-rose-300/40 to-fuchsia-300/40 blur-xl"
          />
        </>
      )}
    </div>
...
function GradientArt({
  ...
  // Kombinasi warna DIUBAH dari /academy dan /creative untuk /events
  const map: Record<Palette, { a: string; b: string }> = {
    indigo: {
      a: "from-indigo-300/55 via-rose-300/45 to-cyan-300/40",
      b: "from-fuchsia-300/45 via-sky-300/40 to-amber-300/35",
    },
    violet: {
      a: "from-violet-300/55 via-sky-300/45 to-emerald-300/40",
      b: "from-rose-300/45 via-amber-300/40 to-indigo-300/35",
    },
    emerald: {
      a: "from-emerald-300/55 via-teal-300/45 to-cyan-300/40",
      b: "from-lime-300/45 via-emerald-300/40 to-fuchsia-300/35",
    },
    amber: {
      a: "from-amber-300/55 via-orange-300/45 to-rose-300/40",
      b: "from-cyan-300/45 via-sky-300/40 to-violet-300/35",
    },
  };
...
export default function EventsPage(): React.JSX.Element {
  ...
  const slides = useMemo(
    () => [
      {
        title: "FMG Events & Festivals",
        kicker: "Live • Tour • Showcase • Activations",
        description:
          "End-to-end live engine: we design experiences, run operations, and prove impact with data — from clubs to festivals.",
        headIcon: Ticket,
      ...
      {
        title: "Live Shows",
        kicker: "Production • Stage • Experience",
        description:
          "Design and deliver shows audiences remember: stage, sound, light, crew, and run-of-show dialed in.",
        bullets: [
          "Stage plots, input lists & tech riders",
          "FOH/monitor, lighting & timecode ops",
      ...
      {
        title: "Showcases",
        kicker: "Industry • Press • Fans",
        description:
          "Curated showcases that move careers — with the right A&R, media, and partners in the room.",
        bullets: [
          "Invite lists, RSVPs & guest services",
          "A&R/press coordination & green rooms",
          "Capture: photo/video, live cuts & reels",
          "Follow-up sequences & coverage tracking",
        ] as const,
        headIcon: Stars,
      ...
      },
      {
        ...
        kicker: "Routing • Logistics • Settlement",
        description:
          "Road-ready ops from holds to settlement — transparent budgets and calm execution city to city.",
        bullets: [
          "Routing & market strategy, venue holds",
          "Advancing: backline, travel & hotels",
          "Per diems, crew & vendor management",
          "Nightly settlement & reporting",
        ] as const,
        headIcon: MapPin,
        tint: "amber" as const,
        artDepth: 1.0,
      ...
      {
        title: "Brand Activations (Measurable)",
        kicker: "Experiences • Partners • KPIs",
        description:
          "Concept, produce, and measure activations that earn attention and drive outcomes — not vanity metrics.",
        bullets: [
          "Experience design & onsite production",
          "Co-branded content & creator loops",
          "Lead capture, CRM & offer flows",
          "KPI dashboards, UTM & post-event attribution",
        ] as const,
        headIcon: LineChart,
        tint: "indigo" as const,
        artDepth: 0.95,
    ...
    <main className="relative min-h-screen overflow-hidden bg-white text-neutral-900 dark:bg-neutral-950 dark:text-white">
      ...
      >
        ...
        >
          ...
          >
            <h2 className="text-3xl font-semibold sm:text-4xl md:text-5xl">Book FMG Events.</h2>
            <p className="mx-auto mt-3 max-w-xl text-neutral-700 dark:text-white/80">
              From club shows to festival stages — design, ops, and measurable impact in one team.
            </p>
