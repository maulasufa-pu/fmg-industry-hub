...
/*************************************************
 * FMG Universe /creative – fullpage slides (responsive)
 * - Vertical scroll snap per screen
 * - Right-side nav rail (desktop), bottom dots (mobile)
 * - Animated gradient artwork per slide
 * - Global floating objects with continuous motion
 * - Parallax on scroll for background layers & art
 *************************************************/
...
function GradientArt({
  ...
  return (
    <motion.div
      style={{ y }}
      className="relative mx-auto aspect-square max-w-[16rem] sm:max-w-xs overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl"
    >
      {/* glass highlight */}
      ...
      {/* Icon overlay (white, to “read” the service quickly) */}
      {OverlayIcon && (
        <div className="absolute inset-0 grid place-items-center">
          <div className="rounded-xl bg-black/20 px-3 py-2 backdrop-blur">
...
type SlideProps = {
  ...
  cta?: React.ReactNode;
  // optional service icon (shown small above text)
...
/* ---------- Right-side vertical nav (garis / desktop only) ---------- */
function NavRail({
  total,
  activeIndex,
  onGo,
...
export default function CreativePage(): React.JSX.Element {
    ...
    () => [
      {
        ...
        cta: (
          <>
            ...
            <a href="/contact" className="inline-flex items-center justify-center rounded-xl border border-white/40 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10">Book a call</a>
          </>
        ),
      },
      {
      ...
      },
      {
        ...
        artDepth: 0.9,
        cta: (
          <a href="/quote" className="inline-flex items-center justify-center rounded-xl bg-white px-4 py-2 text-sm font-semibold text-neutral-900 shadow hover:bg-neutral-100">Get a quote</a>
        ),
      },
    ],
    []
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      (entries) => {
        // pick highest ratio intersecting
  ...
  return (
    <main className="relative min-h-screen bg-neutral-950 text-white overflow-hidden">
      ...
      {/* Right-side vertical nav (garis / desktop) */}
      ...
      >
        ...
        <section
          ...
          className="relative grid min-h-screen snap-start place-items-center px-4 py-20 sm:py-24 sm:px-8"
        >
          {/* Tambah sedikit parallax di art kecil */}
          <div className="absolute inset-0 -z-10">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.5 }}
              transition={{ duration: 0.8 }}
              className="absolute left-10 top-10 h-36 w-36 rounded-full bg-gradient-to-br from-fuchsia-400/40 via-rose-300/30 to-amber-300/30 blur-2xl"
            />
            <motion.div
              animate={{ rotate: [0, 360] }}
              transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
              className="absolute right-10 bottom-10 h-48 w-48 rounded-full bg-[conic-gradient(var(--tw-gradient-stops))] from-white/5 via-white/20 to-white/5 blur-xl"
            />
          </div>
          ...
          >
            ...
            <p className="mx-auto mt-3 max-w-xl text-white/80">
              Tell us about your project and timeline—we’ll reply with a scope,
              approach, and transparent quote.
            </p>
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              <a
                href="/contact"
                className="inline-flex items-center justify-center rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-neutral-900 shadow hover:bg-neutral-100"
              >
