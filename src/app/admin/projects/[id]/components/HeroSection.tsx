...
export default function HeroSection({ 
  ...
  return (
    <>
        ...
        <div className="relative z-10 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-8">
          ...
          {showRightActions && (
            ...
            >
              ...
              >
                <motion.div 
                  className="absolute inset-0 bg-gradient-to-r from-emerald-600 via-green-600 to-teal-600" 
                  initial={{ x: "-100%" }} 
                  whileHover={{ x: 0 }} 
              ...
              >
                <motion.div 
                  className="absolute inset-0 bg-gradient-to-r from-amber-600 via-orange-600 to-red-600" 
                  initial={{ x: "-100%" }} 
                  whileHover={{ x: 0 }} 
                  transition={{ type: "tween", duration: 0.3 }} 
                />
      ...
      {/* Mobile Quick Actions - Only show if user has access */}
      {showRightActions && (
        <motion.div 
          className="flex sm:hidden grid grid-cols-2 gap-4" 
          initial={{ opacity: 0, y: 20 }} 
          animate={{ opacity: 1, y: 0 }} 
        ...
        >
          ...
          <motion.button 
            onClick={onPutOnHold}
            className="rounded-xl bg-gradient-to-r from-orange-500 to-red-600 px-4 py-3 text-white font-medium shadow" 
            whileHover={{ scale: 1.05 }} 
            whileTap={{ scale: 0.95 }}
          >
            ⏸️ Put on Hold
